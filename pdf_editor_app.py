...
def hex_to_rgba_tuple(hex_color: str, alpha_0_255: int):
    """Return RGBA tuple from #RRGGBB and alpha(0-255)."""
    h = hex_color.lstrip("#")
    r = int(h[0:2], 16); g = int(h[2:4], 16); b = int(h[4:6], 16)
    return (r, g, b, max(0, min(255, alpha_0_255)))

def hex_with_alpha(hex_color: str, alpha_0_255: int) -> str:
    """Return #RRGGBBAA string for fabric.js color format used by st_canvas."""
    h = hex_color.lstrip("#")
...
def save_as_pptx(images):
    ...
    for im in images:
        ...
        b = io.BytesIO()
        (im.convert("RGB") if im.mode != "RGB" else im).save(b, format="PNG")
        b.seek(0)
...
st.title("📄 Streamlit PDF 편집기 (opacity 지원)")
...
st.markdown("**PDF 업로드 → 순서/삭제 → 도형/펜/텍스트(투명도 지원) → 저장(PDF/PNG/JPG/PPTX)**")
...
with left:
    ...
    stroke_alpha_pct = st.slider("선 투명도(%)", 0, 100, 100, help="0=완전 투명, 100=불투명")
    fill_color = st.color_picker("채우기 색상(도형용)", "#000000")
    fill_alpha_pct = st.slider("채우기 투명도(%)", 0, 100, 50, help="0=완전 투명, 100=불투명")

    # convert % to 0-255 alpha and attach to hex (#RRGGBBAA)
    ...
    canvas_result = st_canvas(
        ...
        background_image=np.array(page_img),
...
st.caption("※ 팁: 선/채우기 색상은 투명도(%)를 따로 조절할 수 있습니다. 페이지가 많으면 작업 해상도를 낮춰 속도를 높이세요.")
